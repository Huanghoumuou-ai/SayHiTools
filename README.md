...
- 商品图自动等比缩放，不裁切、不拉伸。
- 默认输出画布为 `1440 x 1440`。
- 商品图默认放入画布中心 `1080 x 1080` 区域，也可以自定义商品图区域大小和中心位置。
- 支持选择背景色，默认白色。
...
3. 将商品图等比缩放到指定商品区域内，默认区域为画布中心 `1080 x 1080`。
...
- 商品图位置：画布中心，可自定义中心 X / 中心 Y
- 输出格式：`PNG`

如果商品图区域大于输出画布，系统会自动限制在画布范围内，仍然保持等比缩放、不裁切。
...
| `product_area_width` | int | `1080` | 商品图适配区域宽度 |
| `product_area_height` | int | `1080` | 商品图适配区域高度 |
| `product_center_x` | int | 自动居中 | 商品图区域中心 X，可选 |
| `product_center_y` | int | 自动居中 | 商品图区域中心 Y，可选 |
